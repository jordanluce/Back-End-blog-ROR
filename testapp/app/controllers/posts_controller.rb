...
after_filter :clear_sessions, only: [:show]
  ...
  def index
  	if params[:tag]
  		@posts = Post.filter_by_tags(params[:tag]).page(params[:page]).per(Setting.first.try(:post_per_page))
  	else
  	@posts = Post.published.page(params[:page]).per(Setting.first.try(:post_per_page))
  	end
  end
